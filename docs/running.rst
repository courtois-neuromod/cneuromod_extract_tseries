...
We use `Hydra <https://hydra.cc/>`to flexibly combine datasets, parcellation
...
the `compression level can be set <https://docs.h5py.org/en/stable/high/dataset.html>` with `compression_opts` = [0, 10[.
...
`nilearn's load_confounds_strategy <https://nilearn.github.io/dev/modules/generated/nilearn.interfaces.fmriprep.load_confounds_strategy.html>`.
