...
def split_episodes(
    ...
    for ses in sub_h5:
        test_set += [
            x for x in sub_h5[ses] if x.split('-')[-1][:3] == 's03'
        ]
    ...
    for ses in sub_h5:
        train_set += [
            x for x in sub_h5[ses] if x.split('-')[-1][:3] not in ['s03', val_season]]
...
def export_images(
    ...
    atlas_path = Path(
        ...
        f"{subject}_task-friends_space-MNI152NLin2009cAsym_atlas-{atlas}_"
    ...
    for s in ['train', 'val']:
        ...
        nib.save(
            nii_file,
            f"{odir}/{subject}_{atlas}_{parcel}_RidgeReg{modal_names}_R2_{s}.nii.gz",
        )
...
def test_ridgeReg(
    ...
    if "text" in modalities:
        ...
        for feat in text_features:
            m += f"_{t_feat[feat]}"
    with open(f"{odir}/{subject}_ridgeReg{m}_{atlas}_{parcel}_result.json", 'w') as fp:
