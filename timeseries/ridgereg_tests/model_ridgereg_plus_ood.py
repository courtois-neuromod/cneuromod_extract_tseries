...
def get_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Conducts ridge regression on parcelated OOD dataset."
        ),
    )
    parser.add_argument(
        "--idir",
...
def split_episodes(
    ...
    sub_h5 = h5py.File(
        ...
        f"{subject_id}_task-ood_space-MNI152NLin2009cAsym_"
        f"atlas-{atlas}_desc-{parcel}_timeseries.h5",
        "r",
    )

    # for each movie, use run 1 as train and run 2 as test set
    train_set = []
    val_set = []
    for ses in sub_h5:
        train_set += [x for x in sub_h5[ses] if x.split("_")[1][-1] == '1']
        val_set += [x for x in sub_h5[ses] if x.split("_")[1][-1] == '2']
...
def build_input(
    ...
    av_modalities = [x for x in modalities if x in ["visual", "audio"]]
...
def build_output(
    ...
    for i, run in enumerate(runs):
        ses = run.split("_")[0]
        run_ts = np.array(sub_h5[ses][run])[start:, :]
...
def export_images(
    ...
    """
    atlas_path = Path(
        f"{adir}/{atlas}_{parcel}/{subject}/func/"
        f"{subject}_task-ood_space-MNI152NLin2009cAsym_atlas-{atlas}_"
        f"desc-{parcel}_dseg.nii.gz"
    )
    ...
    for s in ['train', 'val']:
        ...
        nib.save(
            nii_file,
            f"{odir}/{subject}_dset-OOD_{atlas}_{parcel}_RidgeReg{modal_names}_R2_{s}.nii.gz",
        )
...
def test_ridgeReg(
    ...
    if "text" in modalities:
        ...
        for feat in text_features:
            m += f"_{t_feat[feat]}"
    with open(f"{odir}/{subject}_dset-OOD_ridgeReg{m}_{atlas}_{parcel}_result.json", 'w') as fp:
        json.dump(res_dict, fp)

    # export parcelwise scores as .nii images for visualization
    if adir is not None:
        export_images(odir, adir, subject, atlas, parcel, m, res_dict)
...
def main() -> None:
    ...
    train_runs, val_runs = split_episodes(
        args.bdir,
        args.atlas,
        args.parcel,
        args.participant,
