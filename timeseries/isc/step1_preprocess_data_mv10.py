...
def get_lists(args):
    ...
    bold_list = sorted(glob.glob(
        f"{args.idir}/sub-{snum}/ses-0*/func/sub-{snum}_ses-*_task-*_"
    ...
    out_file = Path(
        f"{args.odir}/input/sub-{args.subject}_task-movie10_space_{args.space}_"
...
def save_clean(args, seg_id, clean_bold, out_file):
    """."""

    if "run" in seg_id:
        sid = seg_id.split("_")
        mv_name = f"{sid[0][:-2]}_{sid[1]}"
        seg_id = sid[0]
    else:
        mv_name = seg_id[:-2]
    ...
    with h5py.File(out_file, flag) as f:
        group = f.create_group(mv_name) if not mv_name in f else f[mv_name]
